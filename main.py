...
def explore(h, interval):
    if h - interval * 2 <= 0:
        h_lst = [interval * (i + 1) for i in range(5)]
    elif h + interval * 2 >= 1:
        h_lst = [1 - interval * (i + 1) for i in range(5)]
    else:
        h_lst = [h - 2 * interval, h - interval, h, h + interval, h + 2 * interval]
    return h_lst

def evaluate1(net): #用cross entropy 作为评价，值越小越好, 最后加负号就是越大越好
    loss = nn.CrossEntropyLoss()
    input = net(train_data)
    return -loss(input, train_label).item()


def evaluate2(net): #单纯用accuracy作为evaluation
...
for i in range(n_folds):
    ...
    nets = [Model(config) for _ in range(25)]
    population = [(net, evaluate1(net), DROPOUT_INIT, LEARNING_RATE_INIT) for net in nets]
    ...
    for e in range(EPOCH):
        ...
        dropout_lst = explore(elite[2], DROPOUT_INTERVAL)
        learning_rate_lst = explore(elite[3], LEARNING_RATE_INTERVAL)

        population = []
        for dr in dropout_lst:
            for lr in learning_rate_lst:
                config = Config(pretrained_embeddings, dr)
                net = Model(config)
                net.load_state_dict(elite_net.state_dict())
                optimizer = optim.Adadelta(net.parameters(), lr=lr)
                step(train_data, train_label, BATCH_SIZE, net, optimizer)
                if e > EPOCH/2:
                    population.append((net, evaluate2(net), dr, lr))
                else:
                    population.append((net, evaluate1(net), dr, lr))
