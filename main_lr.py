...
for i in range(n_folds):
    ...
    config = Config(pretrained_embeddings, DROPOUT)
    nets = [Model(config) for _ in range(POPULATION_SIZE)]
    population = [(net, evaluate1(net), LEARNING_RATE_INIT) for net in nets]
    ...
    for e in range(EPOCH):
        ...
        loss_lst.close()
        lr_lst.close()
        learning_rate_lst = explore(elite[2], POPULATION_SIZE, LEARNING_RATE_INTERVAL)
        ...
        for j in range(POPULATION_SIZE):
            ...
            optimizer = optim.Adadelta(net.parameters(), lr=learning_rate_lst[j])
            step(train_data, train_label, BATCH_SIZE, net, optimizer)
            if e > EPOCH/2:
                population.append((net, evaluate2(net), learning_rate_lst[j]))
            else:
                population.append((net, evaluate1(net), learning_rate_lst[j]))
